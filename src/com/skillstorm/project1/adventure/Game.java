...
public class Game {
	...
	int numFightsMax = 3; // How many rounds of fight
	...
	public void start() {
		...
		System.out.println("Welcome to the game, where you can't loose! " + player.getName() + " the " + player.getClassName() + "!");
...
// Start the game loop
		while (true) {
			...
			scanner.nextLine(); // User decision

			if (player instanceof Player.Warrior) {
				if (choice == 1) {
					System.out.println("You fought bravely. Due to your immense stringth, have defeated the "
...
//					break;
				...
				if (choice == 1) {
					System.out.println("You bravely try to fight but having no strength " + monsters[currentRoom]
...
//					continue;
			...
			if (numFights == numFightsMax) { // check if 3 fights have been completed
				System.out.println(
						"*******Congratulations, you have cleared all the monsters and reached the last room*******");
				System.out.println(
						"*******You have reached the exit! You are Free!*******");
				gameOver(scanner);
				break;
			}
